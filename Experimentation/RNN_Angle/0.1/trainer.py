...
class Trainer():
    ...
    def training(self, X, Y, epoch):
        ...
        feed_dict={ self.agent.X: X, 
                    ...
                    self.agent.LabelSine: Y[:, :, 1],
                    self.agent.LabelCosine: Y[:, :, 2],
        ...
        _, meanLoss, normLoss, sineLoss, cosineLoss =self.agent.sess.run([self.agent.opt, self.agent.meanLoss, self.agent.errorNorm, self.agent.errorSine, self.agent.errorCosine], feed_dict=feed_dict)
