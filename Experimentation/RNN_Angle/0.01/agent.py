...
class Network():
    ...
    def buildNetwork(self):
        ...
        with tf.compat.v1.variable_scope("Sine"):
            self.W5=tf.compat.v1.get_variable("Weights_LinearDecoder_Sine", [self.ThirdLayer_units, 1], initializer=tf.contrib.layers.xavier_initializer())
            self.B5=tf.compat.v1.get_variable("Biases_LinearDecoder_Sine", [1], initializer=tf.contrib.layers.xavier_initializer())

            self.OutputSine=tf.matmul(self.nonlinear3, self.W5) + self.B5

        with tf.compat.v1.variable_scope("Cosine"):
            self.W6=tf.compat.v1.get_variable("Weights_LinearDecoder_Cosine", [self.ThirdLayer_units, 1], initializer=tf.contrib.layers.xavier_initializer())
            self.B6=tf.compat.v1.get_variable("Biases_LinearDecoder_Cosine", [1], initializer=tf.contrib.layers.xavier_initializer())

            self.OutputCosine=tf.matmul(self.nonlinear3, self.W6) + self.B6
    ...
    def buildTraining(self):
        ...
        #Compute the l2_loss
        l2_loss = self.weight_decay*tf.compat.v1.nn.l2_loss(self.W4) + self.weight_decay*tf.compat.v1.nn.l2_loss(self.W5) + self.weight_decay*tf.compat.v1.nn.l2_loss(self.W6)
        ...
        self.meanLoss = self.errorNorm + l2_loss + 0.01*self.errorSine + 0.01*self.errorCosine
