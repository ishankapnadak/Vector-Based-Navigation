...
def showGridCells(agent, dataGenerator, num_traj, num_steps, llu, slu, tlu, bins):
    factor=2.2/bins
    activityMap1 = np.zeros((llu, bins, bins))
    activityMap2 = np.zeros((slu, bins, bins))
    activityMap3 = np.zeros((tlu, bins, bins))
    counter = 0
    ...
    for startB in range(0, num_traj, batch_size):
        ...
        linearNeurons1, linearNeurons2, linearNeurons3 = agent.sess.run([agent.linearLayer, agent.linearLayer2, agent.linearLayer3], feed_dict=feed_dict)
        #print(linearNeurons.shape)
        #-----------------------------------------------------#


        #Convert 500,100,2 -> 50000,2
        posReshaped=np.reshape(positions[startB:endB],(-1,2))
        ...
        for t in range(linearNeurons1.shape[0]):
            ...
            activityMap3[:,bin_x, bin_y]+=np.abs(linearNeurons3[t])
            counter += 1
    ...
    normMap3=(result3 -np.min(result3))/(np.max(result3)-np.min(result3))
    ...
    for i in range(tlu):
        fig.add_subplot(rows, cols, i+1)
        #normMap=(result[i]-np.min(result[i]))/(np.max(result[i])-np.min(result[i]))
        plt.imshow(normMap3[i], cmap="jet", origin="lower")
        plt.axis('off')

    fig.savefig('activityMaps/neurons3.jpg')
