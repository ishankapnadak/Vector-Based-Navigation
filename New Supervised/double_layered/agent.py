...
class Network():
    ...
    def buildNetwork(self):
        ...
        with tf.compat.v1.variable_scope("Norm"):
            ...
            self.OutputNorm=tf.matmul(self.linearLayer2Drop, self.W3) + self.B3

        with tf.compat.v1.variable_scope("Phase"):
            self.W4=tf.compat.v1.get_variable("Weights_LinearDecoder_Phase", [self.SecondLayer_units, 1], initializer=tf.contrib.layers.xavier_initializer())
            self.B4=tf.compat.v1.get_variable("Biases_LinearDecoder_Phase", [1], initializer=tf.contrib.layers.xavier_initializer())   
            
            #Compute the predicted Head-direction Cells Distribution
            self.OutputPhase=tf.matmul(self.linearLayer2Drop, self.W4) + self.B4
    ...
    def buildTraining(self):
        ...
        #Compute the l2_loss
        l2_loss = self.weight_decay*tf.compat.v1.nn.l2_loss(self.W4) + self.weight_decay*tf.compat.v1.nn.l2_loss(self.W3)
