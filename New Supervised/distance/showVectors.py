...
def showVectors(agent, dataGenerator, num_traj, num_steps, llu, slu, tlu, bins):
    ...
    for startB in range(0, num_traj, batch_size):
        ...
        for startT in range(0, num_steps, 100):
            ...
            norm = norm[0].reshape((100))
            #print(norm)
            #print(norm.shape)
            #norm_end = norm[99]
            hist[startB][startT : endT] = norm
            ...
            #X_direct = norm_end * np.cos(phase_true)
            #Y_direct = norm_end * np.sin(phase_true)

            #ax.quiver(current_X, current_Y, X_direct, Y_direct, width=0.005, angles='xy', scale_units='xy', scale=1, color='g')

            #circle = plt.Circle((current_X, current_Y), norm, color='r', fill=False)
            #ax.add_patch(circle)
