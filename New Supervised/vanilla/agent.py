...
class Network():
    def __init__(self, session, lr, hu, lu, clipping, weightDecay, batch_size, num_features, n_steps):
        self.sess=session
    ...
    def buildNetwork(self):
        ...
        with tf.compat.v1.variable_scope("Norm"):
            self.W2=tf.compat.v1.get_variable("Weights_LinearDecoder_Norm", [self.LinearLayer_units, 1], initializer=tf.contrib.layers.xavier_initializer())
            self.B2=tf.compat.v1.get_variable("Biases_LinearDecoder_Norm", [1], initializer=tf.contrib.layers.xavier_initializer())
            
            #Compute the predicted Place Cells Distribution
            self.OutputNorm=tf.matmul(self.linearLayerDrop, self.W2) + self.B2

        with tf.compat.v1.variable_scope("Phase"):
            self.W3=tf.compat.v1.get_variable("Weights_LinearDecoder_Phase", [self.LinearLayer_units, 1], initializer=tf.contrib.layers.xavier_initializer())
            self.B3=tf.compat.v1.get_variable("Biases_LinearDecoder_Phase", [1], initializer=tf.contrib.layers.xavier_initializer())   
            
            #Compute the predicted Head-direction Cells Distribution
            self.OutputPhase=tf.matmul(self.linearLayerDrop, self.W3) + self.B3
    ...
    def buildTraining(self):
        ...
        self.LabelPhase = tf.compat.v1.placeholder(tf.float32,  shape=[None, 100], name="Label_Phase")
        
        self.reshapedNorm = tf.reshape(self.LabelNorm, (-1, 1))
        self.reshapedPhase = tf.reshape(self.LabelPhase, (-1, 1))
        ...
        self.errorPhase = tf.compat.v1.reduce_mean(tf.abs(tf.atan2(tf.sin(self.reshapedPhase - self.OutputPhase), tf.cos(self.reshapedPhase - self.OutputPhase))))
        ...
        self.meanLoss = self.errorNorm + self.errorPhase + l2_loss
