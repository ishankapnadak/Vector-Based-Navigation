...
def showGridCells(agent, dataGenerator, num_traj, num_steps, llu, bins):
    factor=2.2/bins
    ...
    for startB in range(0, num_traj, batch_size):
        ...
        for startT in range(0, num_steps, 100):
            ...
            for t in range(linearNeurons.shape[0]):
                ...
                activityMap[:,bin_x, bin_y]+=np.abs(linearNeurons[t])#linearNeurons must be a vector of 512
                counter += 1

    #counterActivityMap[counterActivityMap==0]=1
    #Compute average value
    result=activityMap/counter
