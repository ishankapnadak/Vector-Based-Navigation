...
def trainAgent(agent):
    ...
    while (global_step<SGDSteps):
        ...
        labelData=np.zeros((num_trajectories, numberSteps, 2))
        home_location = pos[:,0]
        for t in range(numberSteps):
            labelData[:,t, 0]=dataGenerator.computeNorm(home_location, pos[:,t])
            labelData[:,t, 1]=dataGenerator.computeAngle(home_location, pos[:,t])
...
if __name__ == '__main__':
    ...
    with tf.compat.v1.Session() as sess:
        try:
            agent=Network(sess, lr=learning_rate, hu=LSTMUnits, lu=linearLayerUnits, slu=secondLayerUnits, clipping=clipping, weightDecay=weightDecay, batch_size=batch_size, num_features=num_features, n_steps=numberSteps)
            ...
            if(args.mode=="train"):    
                ...
                showGridCells(agent, dataGenerator, showCellsTrajectories, numberSteps, linearLayerUnits, secondLayerUnits, bins)
            elif(args.mode=="showvectors"):
                showVectors(agent, dataGenerator, showVectorsTrajectories, numberSteps, linearLayerUnits, secondLayerUnits, bins)
