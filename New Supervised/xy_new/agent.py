...
class Network():
    def __init__(self, session, lr, hu, slu, tlu, lu, clipping, weightDecay, batch_size, num_features, n_steps):
        self.sess=session
    ...
    def buildNetwork(self):
        self.X=tf.compat.v1.placeholder(tf.float32, shape=[None, self.n_steps, self.num_features], name="input") #placeholder for input
        ...
        with tf.compat.v1.variable_scope("Linear_Decoder"):
            ...
            #self.nonlinear1 = tf.compat.v1.nn.tanh(self.linearLayer)
        ...
        with tf.compat.v1.variable_scope("Linear_Decoder_2"):
            self.W2=tf.compat.v1.get_variable("Weights_Linear_to_Linear2", [self.LinearLayer_units, self.SecondLayer_units], initializer=tf.contrib.layers.xavier_initializer())
            self.B2=tf.compat.v1.get_variable("Biases_LSTM_Linear_to_Linear2", [self.SecondLayer_units], initializer=tf.contrib.layers.xavier_initializer())
            ...
            self.nonlinear2 = tf.compat.v1.nn.tanh(self.linearLayer2)
            
            #Compute Linear layer and apply dropout
            #self.linearLayer2Drop=tf.compat.v1.nn.dropout(self.linearLayer2, self.keepProb)

        with tf.compat.v1.variable_scope("Linear_Decoder_3"):
            self.W3 = tf.compat.v1.get_variable("Weights_Linear2_to_Linear3", [self.SecondLayer_units, self.ThirdLayer_units], initializer=tf.contrib.layers.xavier_initializer())
            self.B3 = tf.compat.v1.get_variable("Biases_LSTM_Linear2_to_Linear3", [self.ThirdLayer_units], initializer=tf.contrib.layers.xavier_initializer())

            self.linearLayer3 = tf.matmul(self.nonlinear2, self.W3) + self.B3
            self.nonlinear3 = tf.compat.v1.nn.tanh(self.linearLayer3)

        with tf.compat.v1.variable_scope("X"):
            self.W4=tf.compat.v1.get_variable("Weights_LinearDecoder_X", [self.ThirdLayer_units, 1], initializer=tf.contrib.layers.xavier_initializer())
            self.B4=tf.compat.v1.get_variable("Biases_LinearDecoder_X", [1], initializer=tf.contrib.layers.xavier_initializer())
            
            #Compute the predicted Place Cells Distribution
            self.OutputX=tf.matmul(self.nonlinear3, self.W4) + self.B4

        with tf.compat.v1.variable_scope("Y"):
            self.W5=tf.compat.v1.get_variable("Weights_LinearDecoder_Y", [self.ThirdLayer_units, 1], initializer=tf.contrib.layers.xavier_initializer())
            self.B5=tf.compat.v1.get_variable("Biases_LinearDecoder_Y", [1], initializer=tf.contrib.layers.xavier_initializer())
            
            #Compute the predicted Place Cells Distribution
            self.OutputY=tf.matmul(self.nonlinear3, self.W5) + self.B5

    def buildTraining(self):
        #Fed the Ground Truth Place Cells Distribution and Head Direction Cells Distribution
        self.LabelX = tf.compat.v1.placeholder(tf.float32, shape=[None, self.n_steps], name="LabelX")
        self.LabelY = tf.compat.v1.placeholder(tf.float32, shape=[None, self.n_steps], name="LabelY")
        
        self.reshapedX = tf.reshape(self.LabelX, (-1, 1))
        self.reshapedY = tf.reshape(self.LabelY, (-1, 1))
        ...
        self.errorX = tf.compat.v1.reduce_mean(tf.compat.v1.losses.mean_squared_error(labels=self.reshapedX, predictions=self.OutputX))
        self.errorY = tf.compat.v1.reduce_mean(tf.compat.v1.losses.mean_squared_error(labels=self.reshapedY, predictions=self.OutputY))
        ...
        #Compute the l2_loss
        l2_loss = self.weight_decay*tf.compat.v1.nn.l2_loss(self.W4) + self.weight_decay*tf.compat.v1.nn.l2_loss(self.W5)
        ...
        self.meanLoss = self.errorX + self.errorY + l2_loss
    ...
    def buildTensorBoardStats(self):
        ...
        self.X_loss = tf.compat.v1.placeholder(tf.float32)
        self.mergeXData = tf.compat.v1.summary.merge([tf.compat.v1.summary.scalar("X_loss", self.errorX)])

        self.Y_loss = tf.compat.v1.placeholder(tf.float32)
        self.mergeYData = tf.compat.v1.summary.merge([tf.compat.v1.summary.scalar("Y_loss", self.errorY)])

        self.avgX = tf.compat.v1.placeholder(tf.float32)
        self.mergeAccuracyXData = tf.compat.v1.summary.merge([tf.compat.v1.summary.scalar("Accuracy_X", self.avgX)])

        self.avgY = tf.compat.v1.placeholder(tf.float32)
        self.mergeAccuracyYData = tf.compat.v1.summary.merge([tf.compat.v1.summary.scalar("Accuracy_Y", self.avgY)])

        #self.norm_loss = tf.compat.v1.placeholder(tf.float32)
        #self.mergeNormData = tf.compat.v1.summary.merge([tf.compat.v1.summary.scalar("norm_loss", self.norm_loss)])

        #self.sine_loss = tf.compat.v1.placeholder(tf.float32)
        #self.mergeSineData = tf.compat.v1.summary.merge([tf.compat.v1.summary.scalar("sine_loss", self.sine_loss)])

        #self.cosine_loss = tf.compat.v1.placeholder(tf.float32)
        #self.mergeCosineData = tf.compat.v1.summary.merge([tf.compat.v1.summary.scalar("cosine_loss", self.cosine_loss)])
