...
class Network():
    ...
    def buildNetwork(self):
        self.X=tf.compat.v1.placeholder(tf.float32, shape=[self.batch_size, self.n_steps, self.num_features], name="input") #placeholder for input

        self.keepProb=tf.compat.v1.placeholder(tf.float32, name="keepProb") #placeholder for dropout probability
        ...
        with tf.compat.v1.variable_scope("Linear_Decoder"):
            self.W1=tf.compat.v1.get_variable("W1", [self.Hidden_units, self.LinearLayer_units], initializer=tf.contrib.layers.xavier_initializer())
            self.B1=tf.compat.v1.get_variable("B1", [self.LinearLayer_units], initializer=tf.contrib.layers.xavier_initializer())
        ...
        with tf.compat.v1.variable_scope("Linear_Decoder_2"):
            self.W2=tf.compat.v1.get_variable("W2", [self.LinearLayer_units, self.SecondLayer_units], initializer=tf.contrib.layers.xavier_initializer())
            self.B2=tf.compat.v1.get_variable("B2", [self.SecondLayer_units], initializer=tf.contrib.layers.xavier_initializer())
        ...
        with tf.compat.v1.variable_scope("Linear_Decoder_3"):
            self.W3 = tf.compat.v1.get_variable("W3", [self.SecondLayer_units, self.ThirdLayer_units], initializer=tf.contrib.layers.xavier_initializer())
            self.B3 = tf.compat.v1.get_variable("B3", [self.ThirdLayer_units], initializer=tf.contrib.layers.xavier_initializer())

            self.linearLayer3 = tf.matmul(self.nonlinear2, self.W3) + self.B3
            self.nonlinear3 = tf.compat.v1.nn.tanh(self.linearLayer3)

        with tf.compat.v1.variable_scope("X_Out"):
            self.W4=tf.compat.v1.get_variable("W4", [self.ThirdLayer_units, 1], initializer=tf.contrib.layers.xavier_initializer())
            self.B4=tf.compat.v1.get_variable("B4", [1], initializer=tf.contrib.layers.xavier_initializer())
            
            #Compute the predicted Place Cells Distribution
            self.OutputX=tf.matmul(self.nonlinear3, self.W4) + self.B4

        with tf.compat.v1.variable_scope("Y_Out"):
            self.W5=tf.compat.v1.get_variable("W5", [self.ThirdLayer_units, 1], initializer=tf.contrib.layers.xavier_initializer())
            self.B5=tf.compat.v1.get_variable("B5", [1], initializer=tf.contrib.layers.xavier_initializer())
    ...
    def buildTraining(self):
        ...
        self.LabelX = tf.compat.v1.placeholder(tf.float32, shape=[self.batch_size, self.n_steps], name="LabelX")
        self.LabelY = tf.compat.v1.placeholder(tf.float32, shape=[self.batch_size, self.n_steps], name="LabelY")
    ...
    def buildTensorBoardStats(self):
        ...
        self.mergeAccuracyXData = tf.compat.v1.summary.merge([tf.compat.v1.summary.scalar("average_error_X", self.avgX)])

        self.avgY = tf.compat.v1.placeholder(tf.float32)
        self.mergeAccuracyYData = tf.compat.v1.summary.merge([tf.compat.v1.summary.scalar("average_error_Y", self.avgY)])
