...
def trainAgent(agent):
    ...
    while (global_step<SGDSteps):
        ...
        for t in range(numberSteps):
            #print(home_location.shape)
            #print(pos[:,t].shape)
            labelData[:,t, 0]=dataGenerator.computeX(home_location, pos[:,t])
            labelData[:,t, 1]=dataGenerator.computeY(home_location, pos[:,t])
