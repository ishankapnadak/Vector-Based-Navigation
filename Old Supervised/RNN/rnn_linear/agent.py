...
class Network():
    ...
    def buildNetwork(self):
        ...
        with tf.compat.v1.variable_scope("LSTM"):
            ...
            self.rnn_cell = tf.compat.v1.nn.rnn_cell.BasicRNNCell(self.Hidden_units, activation = "linear", name="RNN_Cell")
            ...
            self.output, self.hiddenstatevalue = tf.compat.v1.nn.dynamic_rnn(cell=self.rnn_cell, inputs=self.X,  initial_state=self.hidden_state)
        ...
        with tf.compat.v1.variable_scope("Place_Cells_Units"):
            self.W2=tf.compat.v1.get_variable("Weights_LinearDecoder_placeCells", [self.LinearLayer_units, self.PlaceCells_units], initializer=tf.contrib.layers.xavier_initializer())
            self.B2=tf.compat.v1.get_variable("Biases_LinearDecoder_placeCells", [self.PlaceCells_units], initializer=tf.contrib.layers.xavier_initializer())
        ...
        with tf.compat.v1.variable_scope("Head_Cells_Units"):
            self.W3=tf.compat.v1.get_variable("Weights_LinearDecoder_HeadDirectionCells", [self.LinearLayer_units, self.HeadCells_units], initializer=tf.contrib.layers.xavier_initializer())
            self.B3=tf.compat.v1.get_variable("Biases_LinearDecoder_HeadDirectionCells", [self.HeadCells_units], initializer=tf.contrib.layers.xavier_initializer())   
    ...
    def buildTraining(self):
        ...
        self.errorPlaceCells=tf.compat.v1.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=self.reshapedPlaceCells, logits=self.OutputPlaceCellsLayer, name="Error_PlaceCells"))
        self.errorHeadCells= tf.compat.v1.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=self.reshapedHeadCells, logits=self.OutputHeadCellsLayer, name="Error_HeadCells"))
