...
class RatSimulator():
    ...
    def generateTimeTrajectory(self):
      positions_X = np.linspace(0.03, 2.17, num=self.number_steps)
      vel = (2.17 - 0.03) / (self.dt * self.number_steps)
      #position_Y = np.random.uniform(low=0, high=2.2)
      positions = np.array([[x, 1] for x in positions_X])
      velocities = vel * np.ones((self.number_steps))
