...
class RatSimulator():
    ...
    #function to generate a constant trajectory
    def constantTrajectory(self):
        #sample X,Y uniformly
    	X = np.random.uniform(low=0, high=2.2)
    	Y = np.random.uniform(low=0, high=2.2)
        #set positions to be (X,Y) for all time steps
    	positions = np.array([[X,Y] for _ in range(self.number_steps)])
        #set all velocities and angles to be zero
    	velocities = np.zeros(self.number_steps)
    	angles = np.zeros(self.number_steps)
    	angVel = np.zeros(self.number_steps)
    	return velocities, angVel, positions, angles
