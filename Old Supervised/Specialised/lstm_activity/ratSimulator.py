...
class RatSimulator():
    ...
    #function to generate trajectory to monitor LSTM activity
    #trajectory is randomly generated for half the time and remains constant after that
    def LSTMTrajectory(self):
        vel_, angvel_, pos_, ang_ = self.generateTrajectory()
        velocities=np.zeros((self.number_steps)) #initialise velocities list
        angVel=np.zeros((self.number_steps)) #initialise angular velocities list
        positions=np.zeros((self.number_steps, 2)) #initialise positions list
        angle=np.zeros((self.number_steps)) #initialise facing angles list

        limit = self.number_steps // 2
        velocities[:limit] = vel_[:limit]
        angVel[:limit] = angvel_[:limit]
        positions[:limit] = pos_[:limit]
        angle[:limit] = ang_[:limit]

        position_final = positions[limit-1]
        positions[limit:] = position_final

        angle_final = angle[limit-1]
        angle[limit:] = angle_final
    ...
    def computeRot(self,ang, pos):
        #computes change in angle when agent is near a wall
        rot=0
